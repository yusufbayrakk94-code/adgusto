...
Deno.serve(
  withInternalAccessHandler(async (req, _user, _whitelist) => {
    ...
    const campaigns = googleAdsResponse.data.results?.map((result: {
      ...
      const conversionsValue = result.metrics?.conversionsValue || 0;
      ...
      return {
        ...
        budget: (result.campaignBudget?.amountMicros || 0) / 1_000_000,
        metrics: {
          ...
          averageCpc: (result.metrics?.averageCpc || 0) / 1_000_000,
          cpa,
          roas,
        },
      };
